...
![image-20230422090009008](images/image-20230422090009008.png)

同步数据的条件，为空则表示同步全部的记录，填充在delete本地表和select Federated表之后，

注意：

1）where中的字段必须同时在本地表和Federated表中；

为了解决本地表和Federated表字段名不同的问题，可以用两个where参数，一个用于Feaderated表的查询，一个用于本地表的删除，在实际项目中Feaderated表和本地表的结构是相同的，就可以不考虑这种情况

2）不要用系统时间作为条件。

在同步之前把where中的时间函数替换成具体的值

我们现在不一定需要优化他，因为不分批同步数据的方法业务上用于处理数据量很小的表，记录数在一万以下即然数据量不大，那就不带条件全表刷新，这种问题就不会有，后面的分批同步和增量同步也不会有
...
如果这两个参数为空就查找数据字典用本地表的字段列表填充他

![image-20230422083656935](images/image-20230422083656935.png)

在主函数中添加

![image-20230422083756515](images/image-20230422083756515.png)

2、业务实现
---

![image-20230422085723879](images/image-20230422085723879.png)
