...
- 远程表的数据只有插入，没有修改和删除操作
- 远程表一定要有自增字段

过程
---

增量同步肯定是分批的，所以不分批同步的代码需要删除

增量同步是根据自增字段来复制数据的，所以每次同步之前从本地表中把自增字段的最大值取出来放到maxkeyvalue全局变量中

再从远程表中查找自增字段的值大于maxkeyvalue的记录，把那些数据同步过来就行

1、修改帮助
---

![image-20230422095539491](images/image-20230422095539491.png)

需要增加这个参数，执行同步的时间间隔单位是秒，刷新同步，没有这个参数程序，每次运行只执行一次同步。然后就退出去了，由调度程序的参数决定同步数据的周期，为什么不让刷新同步程序？常驻内存呢，因为没这个必要，刷新同步数据的周期一般比较长，比如说。站点参数表几分钟同步一次就可以了，没必要几秒钟就刷新一次，还有那些数据量比较大的表。几分钟或者几个小时或者一天才同步一次，要看业务的需求，但是刷新同步的时间间隔。肯定不会太短，增量同步就不一样了，比如说站点观测数据业务需求是越快越好。同步数据的时间间隔可以设置为几秒到几十秒，还有真正同步是根据自自增字段的查找数据的。有数据就同步，没数据什么都不做，对数据库造成的压力非常小，所以增量同步程序是常驻内存中的每间隔若干秒。执行一次同步时间间隔的大小，要看业务需求，

![image-20230422095830299](images/image-20230422095830299.png)

2、增加同步时间间隔的参数
---

执行同步时间间隔跟程序的超时时间这两个参数需要协调，程序的超时时间肯定要比同步时间间隔要大

![image-20230422100407089](images/image-20230422100407089.png)

![image-20230422100540386](images/image-20230422100540386.png)

3 声明
---

![image-20230422100642150](images/image-20230422100642150.png)

4 写函数体
---

![image-20230422100754640](images/image-20230422100754640.png)

5 从本地表中把自增字段的最大值取出来
---

![image-20230422101043525](images/image-20230422101043525.png)

6 从远程表中查找自增字段的值大于maxkeyvalue的记录
---

![image-20230422101917185](images/image-20230422101917185.png)

7 让程序每隔一段时间同步
---

现在的同步程序执行一次同步就退出了，让他常驻内存，数据可能在不断的生成中，所以就不sleep，继续进行同步

在这个函数里面先把continue这个变量初始化为false如果有同步到数据，再把它置为true

![image-20230422102652604](images/image-20230422102652604.png)

### 先初始化为false

<img src="images/image-20230422103116662.png" alt="image-20230422103116662" style="zoom: 50%;" />

![image-20230422103443066](images/image-20230422103443066.png)

### 改为true

![image-20230422103331945](images/image-20230422103331945.png)

细节
===

maxcount中每批同步数据越多，它的性能就越高，但超过100取值200或300的话没什么差别，当这个参数取值到100的时候数据库的吞吐能力基本上达到了极限
