...
![image-20230423162437849](images/image-20230423162437849.png)

2 实现
---

### 1）构造函数、析构函数、初始化数据库连接池，初始化锁

![image-20230423163801806](images/image-20230423163801806.png)

### 2）断开数据库连接，销毁锁

![image-20230423163921122](images/image-20230423163921122.png)
...
![image-20230423165319524](images/image-20230423165319524.png)

### 4）归还数据库连接。 ii不知道是++ii 还是ii++

![image-20230423183956637](images/image-20230423183956637.png)

### 5）检查数据库连接池，断开空闲的连接，在服务程序中，用一个专用的子线程调用此函数。      ii不知道是++ii 还是ii++

![image-20230423184032084](images/image-20230423184032084.png)

### 6）声明连接池对象

![image-20230423180811404](images/image-20230423180811404.png)

### 7）修改线程主函数代码

![image-20230423180719012](images/image-20230423180719012.png)![image-20230423181003552](images/image-20230423181003552.png)

### 8）连接词的检查

在初始化连接词之后创建一个线程，专门负责连接词的检查

![image-20230423181344444](images/image-20230423181344444.png)

写检查数据库连接的函数

![image-20230423181410331](images/image-20230423181410331.png)
...
连接词用于多线程的服务程序，但是，我们并不需要用一个多线程的服务程序来测试它

![8DB801C5A41D28057E599A2B94ABCB44](images/8DB801C5A41D28057E599A2B94ABCB44.png)段错误‼️
---

这是多线程中很容易遇到的一个坑，

![image-20230423182842291](images/image-20230423182842291.png)

我们先来看进程主函数。在主进程中，创建线程之后。会把现成的ID放入容器中。

![image-20230423183447270](images/image-20230423183447270.png)

在线程清理函数中。线程退出的时候，调用线程清理函数，在线程清理函数中把自己的ID从容器中删除。注意特别注意，如果线程跑的太快，主进程可能还来不及把线程的ID放入容器。那么，这里就会出现找不到现成ID的情况。

![image-20230423183621126](images/image-20230423183621126.png)

在进程退出的信号处理函数中。如果线程退出的时候没有把自己从容器中删掉，那么这行代码就会出问题。因为容器中的这个线程根本已经不存在了。

![image-20230423183721324](images/image-20230423183721324.png)那怎么解决这个问题嘞？这个事情还真的不太好办，但是在实际开发中，线程不会这么快退出的，在我们这个程序中，如果线程取数据库连接时失败。就会很快退出。在线程主函数中加上这行代码，就可以了

![image-20230423183834786](images/image-20230423183834786.png)
