采用每连接每线程的方式实现了数据总线的基本功能，但服务程序消耗的资源比较多，性能比较低

每连接每线程的缺点
===

- Oracle数据库连接占用的内存很多，每创建一个新的连接，需要客户端3M，服务端5M的内存
- Linux每个线程占用越10M的虚拟内存和大于10k的固定内存，线程越复杂，占用的内存越多
- 创建线程(消耗时间非常短，可以忽略不计)和连接数据库需要时间
...
- 在主进程中，预先创建n个数据库连接
- 每个线程需要用到数据库的时候，从数据库连接池中取一个空闲的连接，用完之后归还
- 数据库连接需要用锁保护
...
- CPU1核2G，性能在100次/秒以上
- CPU16核32G，性能可达2000次/秒
- 可以满足普通业务的需求，无法满足互联网大厂的并发需求

过程
===

1 声明数据库连接池的数据机构和操作的函数
---

这里只能用互斥锁，不能用自旋锁，因为如果数据量比较大的话，数据库连接占用的时间可能会比较长，自旋锁不合适

![image-20230423100322968](images/image-20230423100322968.png)
...
![image-20230423150055147](images/image-20230423150055147.png)

### 2）从数据库连接池中获取一个空闲的连接

usleep的目的是如果失败了，不断的尝试一直到成功为止

![image-20230423150551892](images/image-20230423150551892.png)

### 3）释放数据库连接

取地址之后地址相等就释放锁

![image-20230423151153315](images/image-20230423151153315.png)

### 4）断开数据库连接，销毁锁

![image-20230423151352285](images/image-20230423151352285.png)

### 5）在main函数中

初始化数据库连接池

![image-20230423151623119](images/image-20230423151623119.png)

### 6）修改线程主函数

![image-20230423152247889](images/image-20230423152247889.png)
