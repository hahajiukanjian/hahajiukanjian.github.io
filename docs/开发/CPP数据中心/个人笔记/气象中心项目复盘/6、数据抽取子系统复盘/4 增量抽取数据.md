全量抽取：执行不带输入参数的SQL语句，不一定抽取全部的数据，可以加条件，适用于数据源表的数据量不大，适用于业务参数表，比如全国站点参数

增量抽取：要求数据源表有自增字段，执行带输入参数的SQL语句(已抽取数据的最大id)，每次抽取数据后，记录已抽取数据的最大id

keyid是自增字段，每次抽取数据后把最大的keyid保存在incfilename文件里面
...
![image-20230421005846751](images/image-20230421005846751.png)
...
![image-20230421005600034](images/image-20230421005600034.png)

再写这个函数的结构体

![image-20230421005806806](images/image-20230421005806806.png)

- 如果是增量抽取，绑定输入参数（已抽取数据的最大id）。

![image-20230421010030155](images/image-20230421010030155.png)

- 更新自增字段的最大值，把结果集中自增字段与文件拿出来的最大值比较大小，取更大的值

![image-20230421010249308](images/image-20230421010249308.png)

- /把最大的自增字段的值写入starg,incfilename文件中![image-20230421010057582](images/image-20230421010057582.png)

![image-20230421010130403](images/image-20230421010130403.png)
